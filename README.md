...
* 简色 : 单一波长的颜色 , 比如红色, 绿色和蓝色.
* 间色: 只有两种颜色融合而成的颜色 : 
    * 红色 + 绿色 = 黄色
    * 红色 + 蓝色 = 品红
    * 绿色 + 蓝色 = 青色
* 复色: 通过组合一种原色和一种邻近的间色来创建的 (==也就是说, 一共有六种==)
    * 橙色 = 255 红色 + 127 绿色 => 原因: 橙色是`红色`和`黄色`的组合, 所以为了创建橙色, 要增加红色的强度, 并降低绿色的强度.
    * 亮绿色 = 255 绿色 + 127 蓝色 => 原因: 创建该颜色需要混合 `青色`, 和 `绿色`, 因此应该增加绿色与蓝色的比值
    * 蓝紫色 = 127 红色 + 255 蓝色 => 原因: 需要组合品红色和蓝色
    * 黄绿色 = 255 绿 127 红 => 黄色 + 绿色
    * 天蓝色 = 127 绿 255 蓝 => 青色 + 蓝色
    * 玫瑰色 = 255 红 127 蓝 => 品红 + 红色

> 色轮是一个圆圈, 其中相似的`颜色`或`色调`彼此靠近, 而不同的颜色相聚较远. 例如纯红色介于玫瑰色和橙色之间.
> 色轮上相互对立的两种颜色成为`补色`. 如果将两种互补色组合在一起, 他们会产生灰色. 但当他们并排放置时, 这些事颜色会产生强烈的视觉对比==显得更亮==

> 注意, 红色和品红色是邻近色, 非常明亮.  如果在网站上使用不当, 这样的颜色对比==会令人分心==, 比如将文字背景色和文字颜色设置为互补色, 这样文字会很难看清.
> 选择一种颜色作为主要颜色, 并且使用其补充颜色作为强调来提醒访问者注意页面上的某些内容, 是一种更好的方式.

## RGB的十六进制表示

> 十六进制的颜色值以`#`字符开头, 从`0-9`和`A-F`中取六个字符. 第一对字符代表红色, 第二对字符代表绿色, 第三对代表蓝色.
>
> FF 是 255 一般用 7F 来降低颜色强度
